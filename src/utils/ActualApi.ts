...
const serialiseConsoleHint = (value: unknown): string | null => {
...
const classifyActualConsoleOutput = (args: unknown[]): ConsoleClassification => {
    if (args.length === 0) {
    ...
    if (/^Debug data for the operations:?/i.test(trimmed)) {
        const hints = rest.map((entry) => serialiseConsoleHint(entry)).filter((hint): hint is string => Boolean(hint));
    ...
    if (SUPPRESSED_PREFIXES.some((prefix) => trimmed.startsWith(prefix))) {
        return {
            suppress: true,
            message: trimmed,
        };
...
const createConsoleInterceptor =
    ...
    (...args: TArgs): void => {
        const classification = classifyActualConsoleOutput(args);
        ...
        if (logger.getLevel() >= LogLevel.DEBUG) {
            logger.debug(classification.message, classification.hints);
        }
    };
